...
# Our code:
push!(technologies, "BiomassGeenerator")
push!(technologies, "BECCS")
push!(fuels,"Biomass")

# Output Ratios
OutputRatio["BiomassGeenerator","Biomass"] = 1
OutputRatio["BECCS","Power"] = 1

# Input Ratios
InputRatio["BECCS", "Biomass"] = 2

# Emission Ratio
EmissionRatio["BECCS"] = -1

# Max Capacity
MaxCapacity["BiomassGeenerator"] = 20
MaxCapacity["BECCS"] = 999

# Kosten
InvestmentCost["BECCS"]=1
InvestmentCost["BiomassGeenerator"]=0.5
VariableCost["BiomassGeenerator"]=1

InputRatio
OutputRatio
# Farben
colors["BiomassGeneration"] = "#336600"   # Green color for BiomassGeneration
colors["Biomass with Carbon Capture and Storage"] = "#663300"  # Brown color for Biomass with Carbon Capture and Storage
...
DataFrame(Containers.rowtable(value,Capacity; header = [:Technology, :value]))

df_res_production

filter(row -> row.value > 0, df_res_production)
filter(row -> row.value > 0, df_res_capacity)
